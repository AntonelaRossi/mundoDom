...
const bands = [
  ...
  router.get('/api/bands', (req, res) => {
    // contesto al cliente con todo el listado de bandas
    res.json(bands);
  });
  
  router.get('/api/bands/:bandId', (req, res) => {
    // me guardo el id que me enviaron en la URL
    console.log(req.params.bandId)
    const bandId = parseInt(req.params.bandId); //aca lo parsea porque queremos que sea un numero, pero la url es un string
  ...
  router.delete('/api/bands/:bandId', (req, res) => {  //:bandId significa que es un parametro, es variable
    // me guardo el ID de la banda a eliminar
    const toDelete = req.params.bandId; //req.params, parametros del pedido son las cositas dps de la url principal, dps de la barra
  ...
  router.get('/api/bands/:bandId/albums', (req, res) => {
  ...
  router.post('/api/bands/:bandId/albums', (req, res) => {
